...
def get_products(db: Session = Depends(get_db), request: Request = None) -> BaseAPIResponseModel[list[ProductOut]] : 
    ...
    return BaseAPIResponse.success(
        request=request,
        data=[p.dict() for p in products],  # dicts are serializable
...
def get_product_by_id(product_id: int, db: Session = Depends(get_db), request: Request = None) -> BaseAPIResponseModel[ProductOut]:
...
def update_product(product_id: int, product_in: schemas.ProductUpdate = Body(...),db: Session = Depends(get_db), request: Request = None) -> BaseAPIResponseModel[ProductOut]:
